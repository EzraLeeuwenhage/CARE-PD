...
class WandBEvaluationCallback(Callback):
    def __init__(self, cfg, eval_interval=50):
        ...
        self.val_epochs, self.floating_gen_hist, self.floating_gt_hist = [], [], []
        self.foot_disp_gen_hist, self.foot_disp_gt_hist = [], []
        self.anchors = {}
        self.gt_pose_dict, self.gt_h36m_dict, self.gt_key_to_severity = None, None, None
    ...
    def on_validation_epoch_end(self, trainer, pl_module):
        ...
        data_dict = generate_trajectories(
            model=pl_module, dataloader=val_loader, num_steps=self.cfg['sampling']['num_steps'], 
            device=pl_module.device, max_batches=self.cfg['training'].get('eval_batches', -1),
            desc=f"W&B Eval Ep {epoch}", is_joint_model=self.is_joint_model,
        )
        ...
        gen_pose_dict, gen_h36m_dict, gen_key_to_severity = {}, {}, {}
        ...
        if self.gt_pose_dict is None:
            ...
            for i, gt_sev in enumerate(data_dict["severities"]):
                ...
                self.gt_pose_dict[seq_key] = data_dict["gt"]["pose"][i].cpu().numpy()
                self.gt_pose_dict[f"{seq_key}_trans"] = data_dict["gt"]["trans"][i].cpu().numpy()
        ...
        smpl_summary, smpl_cache_data = self.smpl_evaluator.evaluate_from_memory(
            self.gt_pose_dict, gen_pose_dict, self.gt_key_to_severity
        )
        ...
        if self.is_joint_model:
            wandb_logs["eval_metrics/Label_Confusion_Matrix"] = wandb.plot.confusion_matrix(
                probs=None, y_true=data_dict["severities"], preds=data_dict["gen_severities"],
                class_names=["Class 0", "Class 1", "Class 2", "Class 3"], title=f"Label Adherence Matrix (Epoch {epoch})"
            )
            wandb_logs["eval_metrics/Prior_State_Correlation_Matrix"] = wandb.plot.confusion_matrix(
                probs=None, y_true=data_dict["prior_severities"], preds=data_dict["gen_severities"],
                class_names=["Class 0", "Class 1", "Class 2", "Class 3"], title=f"Prior Jump Correlation Matrix (Epoch {epoch})"
            )
        ...
        if not is_overfit:
            ...
            wandb_logs.update({
                "physical_realism/floating_tracking": wandb.plot.line_series(
                    xs=self.val_epochs, ys=[self.floating_gen_hist, self.floating_gt_hist],
                    keys=["Generated", "Ground Truth Baseline"], title="Floating / Skating over Epochs", xname="Epoch"
                ),
                "physical_realism/foot_displacement_tracking": wandb.plot.line_series(
                    xs=self.val_epochs, ys=[self.foot_disp_gen_hist, self.foot_disp_gt_hist],
                    keys=["Generated", "Ground Truth Baseline"], title="Foot Displacement over Epochs", xname="Epoch"
                ),
            ...
            for log_name, filename in visuals.items():
                ...
                if img_path.exists(): wandb_logs[log_name] = wandb.Image(str(img_path))
